...
int main() {
  ...
  std::cout << BOLD BLUE ITALIC
            << "=== Test 1: Creating Valid Forms ===" << RESET << "\n";
  try {
  ...
  for (int i = 0; i < 5; i++) {
    try {
      std::cout << "Trying to create: \"" << invalidForms[i] << "\""
                << std::endl;
      AForm* form = someRandomIntern.makeForm(invalidForms[i], "target");
      // Lines below should not execute
      std::cout << RED << "ERROR: Should have thrown exception!" << RESET
                << std::endl;
      delete form;
    } catch (const Intern::FormNotCreatedException& e) {
  ...
  std::cout << BOLD BLUE ITALIC
            << "=== Test 3: Integration with Bureaucrats ===" << RESET
            << std::endl;
  try {
    ...
    std::cout << RED << "Exception in integration test: " << e.what() << RESET
              << std::endl;
  }

  std::cout << BLUE ITALIC << "\n=== All Tests Complete ===" << RESET
            << std::endl;
  return 0;
}

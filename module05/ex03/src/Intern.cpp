...
Intern::Intern(const Intern& src) {
  ...
  std::cout << BOLD << "Intern Copy Constructor called (nothing to copy)"
            << RESET << std::endl;
}
...
AForm* Intern::makeForm(const std::string& form_name,
  ...
  for (size_t i = 0; i < n_form_types; i++) {
    if (form_types[i].compare(form_name) == 0) {
      ...
      std::cout << GREEN << "Intern created " << form->getName() << RESET
                << std::endl;
      return form;
    }
  }
  std::cout << RED << "Error: " << form_name << " not found" << RESET
            << std::endl;
...
// Array of const pointers to intern functions that return AForm* and take

...
Point::Point(const float x_val, const float y_val)
    : x(Fixed(x_val)), y(Fixed(y_val)) {}

Point::Point(const Point& src) : x(src.x), y(src.y) {}

Point::Point(const std::string& name)
    : x(userInput(name, 'x')), y(userInput(name, 'y')) {
  system("clear");
}
...
float Point::userInput(const std::string& name, const char& type) {
  ...
  while (true) {
    std::cout << GREEN << "Enter " << type << " coordinate for " << name
              << RESET << std::endl;
    ...
    if (input_stream >> coord && input_stream.eof()) {
      if (std::isfinite(static_cast<float>(coord)) && checkFixedRange(coord))
        break;
      std::cerr << RED << "Number out of range for Fixed Point class" << RESET
                << std::endl;
    } else

...
const std::string ScalarConverter::pseudo_types[6] = {"-inff", "+inff", "nanf",
                                                      "-inf",  "+inf",  "nan"};
...
void ScalarConverter::getDecPrecision(const std::string& input) {
  ...
  size_t precision =
      (p_pos == input.npos) ? 1 : input.length() - p_pos - 1 - flt;
  precision =
      (precision < 1) ? 1 : precision;  // set to 1 if no digits after '.'
...
void ScalarConverter::setPseudoValue(const std::string& input) {
  if (input.at(0) == 'n')
    ...
    p_val = input.at(0) == '+' ? std::numeric_limits<double>::infinity()

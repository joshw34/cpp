...
const std::string& Brain::getIdea(int index) const {
  if (index < 0 || index >= this->brainSize) {
    std::cerr << RED << "Error: " << index
              << "is Out of Range. Returning ideas[0]" << RESET << std::endl;
    return this->ideas[0];
  }
...
void Brain::printIdea(int index, const std::string& type) const {
  if (index < 0 || index >= this->brainSize) {
    std::cerr << RED << "Error: " << index << "is Out of Range" << RESET
              << std::endl;
    return;
  }
  std::cout << YELLOW << type << ": " << this->ideas[index] << RESET
            << std::endl;
}

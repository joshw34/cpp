...
Character::Character(const std::string& name) : name(name), droppedMateria(NULL) {
	//std::cout << BOLD << "Character Constructor called for " << name << RESET << std::endl;
	initMatArray(this->materia);
}

Character::Character(const Character& src) : name(src.name), droppedMateria(NULL) {
...
Character::s_droplist::s_droplist(AMateria* m) : materia(m), next(NULL) {
	//std::cout << ITALIC << m->getType() << " added to droppedMateria" << RESET << std::endl;
};
